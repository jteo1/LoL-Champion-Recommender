...
def getPlayerMatchList(region, playerID, api_key):
    ...
    if "matches" not in data.keys():
        print "The input username has no ranked matches"
        sys.exit()
    return data["matches"]
...
def convertChampionIDsToChampions(api_key):
    ...
    for name, info in champions_by_name.iteritems():
        champions_by_id[info["id"]] = name.lower()
...
def main():
    ...
    #Configure argument list options
    parser = argparse.ArgumentParser(description = "Make champion recommendations using master/challenger tier players from NA, EUW, EUNE, and KR")
    parser.add_argument('username', nargs = '?', help = "The username to recommend a champion for. If spaces are in the username, surround the name in quotation marks.")
